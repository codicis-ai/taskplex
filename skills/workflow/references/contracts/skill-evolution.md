...
Lives at `${CLAUDE_PLUGIN_DATA}/evolutions/{skill-name}.json`. Created on first evolution, loaded before every skill invocation.
...
When a skill is invoked (e.g., `/tp` triggers taskplex), the orchestrator checks for `${CLAUDE_PLUGIN_DATA}/evolutions/{skill}.json`. If it exists and has active entries:
...
**Process:**
1. Read `${CLAUDE_PLUGIN_DATA}/evolutions/{skill}.json`
