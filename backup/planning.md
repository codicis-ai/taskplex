...
## Production Impact Detection (for planning agent prompts)

Before spawning the planning agent, check whether the task touches production-sensitive areas. If ANY are true, include in the planning agent prompt:

**Triggers** (grep brief.md + modified files from init):
- Database queries, schemas, or migrations
- API endpoints serving external traffic
- Caching logic, TTLs, or cache invalidation
- Authentication, authorization, or session handling
- Infrastructure configuration (CI/CD, deployment, env vars)
- Retry logic, timeouts, or circuit breakers
- Shared services or libraries consumed by multiple systems

**Prompt addition** (append to planning agent context if triggered):
> "This task touches production-sensitive areas ({list triggers detected}). Include a 'Production Impact Assessment' section in spec.md with: blast radius, rollout strategy, rollback plan, operational risks, and monitoring metrics. See spec template for format."

**If no triggers detected**: Don't mention it. The planning agent's spec template handles the skip case.
...
**After both critics APPROVED**: **set `manifest.criticCompleted = true`** and write manifest to disk. This is enforced by a hard gate — source file writes are blocked until this flag is set.
