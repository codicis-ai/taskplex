...
- **Type correctness**: Run `lsp_diagnostics` on all modified files — type errors are P0 findings, no reading required.
- **Completeness checks**: Use `lsp_find_references` to verify new exports are consumed. Unused exports = incomplete wiring.
- **Impact analysis**: Use `lsp_find_references` on changed function signatures to check whether all callers were updated.
- **Type context**: Use `lsp_hover` on unclear types to understand what a variable actually is, instead of tracing through type definitions manually.

### ast-grep (when `sg` CLI is available)
- **Convention scanning**: ast-grep catches structural violations that grep misses. Example: "all components must use named exports" → `sg -p 'export default $EXPR' --lang tsx src/components/` finds violations without false positives from comments.
- **Pattern compliance**: When `conventions.json` declares patterns (e.g., `stateManagement: "zustand"`), use ast-grep to verify no other state management library is imported: `sg -p 'import $$$from "redux"' --lang typescript .`

## Review Checklist

1. **Batch scan** for convention violations via Grep and ast-grep (80% of issues in 3-5 calls)
...
4. **Correctness** — no bugs, proper error handling, correct types, no security issues. Use `lsp_diagnostics` if available.
5. **Completeness** — all files exist, imports resolve, endpoints exist. Use `lsp_find_references` if available to verify wiring.
