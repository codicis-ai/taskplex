...
**Step 5b — Present to user via `AskUserQuestion`**:

> "Quality profile for this task (determines validation rigor):
>
> 1. **Lean** — build checks only. No review agents. For quick fixes and experiments.
> 2. **Standard** — security review + closure + code review (3 agents). For production code.
> 3. **Enterprise** — all of Standard + hardening (blocking) + compliance + dependency/license audit (5+ agents). For critical infrastructure.
>
> Default for {route} route: **{auto-detected profile}**. Pick a number or press Enter for default."
...
|---------|------------------|-------------------|
| **lean** | None | Build checks only |
| **standard** | security-reviewer, closure-agent, code-reviewer | FAIL blocks commit |
| **enterprise** | security-reviewer, closure-agent, code-reviewer, hardening-reviewer, compliance-agent | FAIL blocks commit, hardening blocking |

**Required review artifacts by profile** (enforced by pre-commit hook):

| Profile | Required Files in `.claude-task/{taskId}/` |
|---------|------------------------------------------|
